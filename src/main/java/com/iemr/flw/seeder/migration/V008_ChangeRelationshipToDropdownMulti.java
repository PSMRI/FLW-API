...
/**
 * Changes the Community Contact Tracing form's "Relationship with community contacts" question
 * (CCT_RELATIONSHIP) from CHECKBOX_MULTI to DROPDOWN_MULTI, so the 7 relationship options render as
 * a multi-select dropdown instead of a checkbox list.
 *
 * Both types submit {@code optionValues} and store one response row per selected option, so
 * existing responses remain valid. Options and their conditions (per-relationship count, Type of
 * Space and hours questions, and "Other" text) are untouched. All other scalar fields are read off
 * the existing row and passed back unchanged, since {@code updateQuestion} overwrites all of them
 * unconditionally.
 */
@Component
public class V008_ChangeRelationshipToDropdownMulti implements FormStructureMigration {
    ...
    public void apply(DynamicFormReconciliationService svc) {
        ...
        SectionQuestion question = svc.question(section, "CCT_RELATIONSHIP")

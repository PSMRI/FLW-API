...
public interface DiagnosticOrderRepo extends JpaRepository<DiagnosticOrder, Long> {
    ...
    // villageId/providerServiceMapId via a joined BenFlowStatus row.
    ...
    // is bucketed only by the retest, not both. b.beneficiaryID (not b.beneficiaryRegID) is
    // selected so callers get the beneficiaryId, matching what other beneficiary-list endpoints return.
    @Query("SELECT b.beneficiaryID FROM DiagnosticOrder o, BenFlowStatus b " +
            "WHERE b.beneficiaryRegID = o.benRegID AND b.deleted = false " +
            "AND o.orderType = :orderType AND o.deleted = false " +
